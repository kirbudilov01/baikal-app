...
Временный сервер уже поднят вручную. Это нейтральный технический домен для TestFlight, не домен партии:
...
- Bundle ID `ru.baikal.vrukah` должен быть свободен/закреплен в той команде, через которую собираем;
